...
export default function ProofGallery3D() {
  ...
  useEffect(() => {
    ...
    // ── Build pentagonal prism orb ──
    // 5 faces arranged around a central axis like a pentagon
    ...
    const faceData: { index: number; color: THREE.Color }[] = []

    // Pentagon geometry: 5 faces evenly spaced
    const RADIUS = 2.2        // distance from center to face
    const FACE_W = 2.6        // face width
    const FACE_H = 3.4        // face height
    ...
    CARDS.forEach((card, i) => {
      ...
      // Plane geometry for the image face
      const geo = new THREE.PlaneGeometry(FACE_W, FACE_H, 1, 1)

      // Border glow mesh (slightly larger, emissive)
      const borderGeo = new THREE.PlaneGeometry(FACE_W + 0.12, FACE_H + 0.12, 1, 1)
      ...
      mesh.userData = { index: i, angle, color: faceColor }
      faceGroup.add(mesh)
      faceMeshes.push(mesh)
      faceData.push({ index: i, color: faceColor })
      ...
      // Top/bottom caps per face (trapezoid feel)
    ...
    const updateCaption = () => {
      const normalized = ((faceGroup.rotation.y % (Math.PI * 2)) + Math.PI * 2) % (Math.PI * 2)
      ...
      if (caption) {
        ...
        captionTimer = setTimeout(() => {
          ...
          const color = '#' + new THREE.Color(CARDS[faceIndex].color).getHexString()
    ...
    const onPointerMove = (e: PointerEvent) => {
      ...
      currentRotY = faceGroup.rotation.y
      updateCaption(-currentRotY)
      lastX = e.clientX
    }

    const onPointerUp = () => {
      isDragging = false
      // Resume auto-rotate slowly
      setTimeout(() => { autoRotateSpeed = 0.004 }, 1200)
    }
    ...
    const animate = () => {
      ...
      if (!isDragging) {
        if (Math.abs(dragVelocity) > 0.0001) {
          ...
          if (Math.floor(t * 10) % 3 === 0) updateCaption()
        currentRotY = faceGroup.rotation.y
      }
  ...
  return (
    <div className="relative" style={{ height: '85vh', minHeight: 560, background: 'transparent' }}>
      ...
      <div style={{
        position: 'absolute',
        inset: 0,
        background: 'radial-gradient(ellipse at center, transparent 50%, rgba(5,5,5,0.7) 100%)',
        pointerEvents: 'none',
        zIndex: 1,
      }} />
    </div>
